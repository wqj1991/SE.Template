...
namespace SE.RemoteRequest.Proxies;

/// <summary>
/// 异步分发代理生成器
/// </summary>
internal static class SeAspectDispatchProxyGenerator
{
    ...
    private static readonly MethodInfo s_dispatchProxyInvokeMethod =
        typeof(SeAspectDispatchProxy).GetTypeInfo().GetDeclaredMethod("Invoke");

    private static readonly MethodInfo s_dispatchProxyInvokeAsyncMethod =
        typeof(SeAspectDispatchProxy).GetTypeInfo().GetDeclaredMethod("InvokeAsync");

    private static readonly MethodInfo s_dispatchProxyInvokeAsyncTMethod =
        typeof(SeAspectDispatchProxy).GetTypeInfo().GetDeclaredMethod("InvokeAsyncT");
    ...
    private static Type GenerateProxyType(Type baseType, Type interfaceType)
    {
        ...
        if (!interfaceType.GetTypeInfo().IsInterface)
        {
            ...
            throw new ArgumentException($"InterfaceType_Must_Be_Interface, {interfaceType.FullName}",
    ...
    private static ProxyMethodResolverContext Resolve(object[] args)
    {
        ...
        var context = new ProxyMethodResolverContext(packed, method);

        var dynamicApiControllerSettings =
            App.GetConfig<DynamicApiControllerSettingsOptions>("DynamicApiControllerSettings", true);


        var serviceName = packed.DeclaringType.Name;

        serviceName = serviceName.ClearStringAffixes(affixes: dynamicApiControllerSettings.AbandonControllerAffixes);
        serviceName = serviceName.ClearStringAffixes(-1, affixes: new[] { "I" });

        // 根据 
        var RequestUrl = dynamicApiControllerSettings.DefaultRoutePrefix + "/" + serviceName + "/" + method.Name;
                         context.RequestUrl = "https://localhost:5001/api/Book/Get/1";


        var defaultVerbToHttpMethods = new ConcurrentDictionary<string, string>
        {
            ["post"] = "POST",
            ["add"] = "POST",
            ["create"] = "POST",
        ...
        // 获取配置的复写映射规则
        var verbToHttpMethods = dynamicApiControllerSettings.VerbToHttpMethods;
        ...
        context.MethodType = HttpMethod.Get;
    ...
    public static object Invoke(object[] args)
    {
        ...
        try
        {
            ...
            returnValue = s_dispatchProxyInvokeMethod.Invoke(context.Packed.DispatchProxy,
                new object[] { context.Method, new object[] { context } });
    ...
    public class PackedArgs
    {
        ...
        internal SeAspectDispatchProxy DispatchProxy => (SeAspectDispatchProxy)_args[DispatchProxyPosition];
    ...
    private class ProxyBuilder
    {
        ...
        private MethodBuilder AddMethodImpl(MethodInfo mi)
        {
            ...
            var mdb = _tb.DefineMethod(mi.Name, MethodAttributes.Public | MethodAttributes.Virtual, mi.ReturnType,
                paramTypes);
            ...
            il.Emit(OpCodes.Ldarg_0);
            packedArr.EndSet(typeof(SeAspectDispatchProxy));
...
public class DispatchProxyHandler
{
    ...
    {
        return SeAspectDispatchProxyGenerator.Invoke(args);
    }
    ...
    {
        return SeAspectDispatchProxyGenerator.InvokeAsync(args);
    }
    ...
    {
        return SeAspectDispatchProxyGenerator.InvokeAsync<T>(args);
    }
}