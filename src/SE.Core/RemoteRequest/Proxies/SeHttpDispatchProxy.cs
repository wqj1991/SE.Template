...
/// </summary>
[SuppressSniffer]
public class SeHttpDispatchProxy : SeAspectDispatchProxy, IDispatchProxy
{
    /// <summary>
    /// 被代理对象
    /// </summary>
    ...
    private HttpRequestPart BuildHttpRequestPart(MethodInfo method, object[] args)
    {
        ...
        var context = ((SeAspectDispatchProxyGenerator.ProxyMethodResolverContext)args[0]);
        ...
        {
            // 根据方法名生成动态地址
            httpRequestPart.SetRequestUrl(context.RequestUrl)
                .SetHttpMethod(context.MethodType)
                .SetTemplates(parameters.ToDictionary(u => u.Name, u => u.Value))
                .SetRequestScoped(Services);
        }
